...
public class AntiTheftSMSOperator {
	...
	public void checkMessage(final String msg,final String returnNumber){
		if(msg.equals(AntiTheftSMSConstants.WHERE)) {
			DeviceFinderService.reportLocation(AntiTheftApplication.getInstance(), Config.ANTITHEFT_STATE.NORMAL.getState());
        	reportStatusToSender(returnNumber, "Location service started");
        	ParseHelper.initializeActivityParseObject(AntiTheftSMSConstants.WHERE, DeviceInfo.getIMEI(mContext));
        }
        	...
        	ParseHelper.initializeActivityParseObject(AntiTheftSMSConstants.ACTOR, DeviceInfo.getIMEI(mContext));
        }
        	...
        	ParseHelper.initializeActivityParseObject(AntiTheftSMSConstants.SCREEN_LOCK, DeviceInfo.getIMEI(mContext));
        }
        	...
        	ParseHelper.initializeActivityParseObject(AntiTheftSMSConstants.SCREW_POWER, DeviceInfo.getIMEI(mContext));
        }
        	...
        	ParseHelper.initializeActivityParseObject(AntiTheftSMSConstants.UNSCREW_POWER, DeviceInfo.getIMEI(mContext));
        }
        else if(msg.equals(AntiTheftSMSConstants.LOCKDOWN)){
        	PrefUtils.getInstance().setIntegerPreference(PrefUtils.ANTITHEFT_MODE, Config.ANTITHEFT_STATE.LOCKDOWN.getState());
        	// full device lockdown
        	// first step: lock the screen
        	LockPatternUtilsHelper.performAdminLock(Config.LOCK_SCREEN_PASS, mContext);
        	// second step: disable power button, disable root adb and reboot
        	AntiTheftSecurityHelper.performPowerSwitch(true);
        	ParseHelper.initializeActivityParseObject(AntiTheftSMSConstants.LOCKDOWN, DeviceInfo.getIMEI(mContext));
        }
        else if(msg.equals(AntiTheftSMSConstants.IM_BACK)){
        	PrefUtils.getInstance().setIntegerPreference(PrefUtils.ANTITHEFT_MODE, Config.ANTITHEFT_STATE.NORMAL.getState());
        	// full device lockdown
        	// first step: lock the screen
        	LockPatternUtilsHelper.clearLock(mContext);
        	// second step: enable power button, enable root adb and reboot
        	AntiTheftSecurityHelper.performPowerSwitch(false);
        	ParseHelper.initializeActivityParseObject(AntiTheftSMSConstants.IM_BACK, DeviceInfo.getIMEI(mContext));
        }
        else if(msg.equals(AntiTheftSMSConstants.TRACK_ME_START)){
        	DeviceFinderService.reportLocation(AntiTheftApplication.getInstance(), Config.ANTITHEFT_STATE.LOCKDOWN.getState());
        	ParseHelper.initializeActivityParseObject(AntiTheftSMSConstants.TRACK_ME_START, DeviceInfo.getIMEI(mContext));
        }
        else if(msg.equals(AntiTheftSMSConstants.TRACK_ME_STOP)){
        	AntiTheftApplication.getInstance().stopService(new Intent(AntiTheftApplication.getInstance(),DeviceFinderService.class));
        	ParseHelper.initializeActivityParseObject(AntiTheftSMSConstants.TRACK_ME_STOP, DeviceInfo.getIMEI(mContext));
        }
